...
func (p *Proxy) InvokeGetRemoteOAuth2ConnectURL(r *incoming.Request) (string, error) {
	...
	state, err := p.store.OAuth2.CreateState(r)
	if err != nil {
		return "", err
	}
...
func (p *Proxy) InvokeCompleteRemoteOAuth2(r *incoming.Request, urlValues map[string]interface{}) error {
	...
	err = p.store.OAuth2.ValidateStateOnce(r, urlState)
	if err != nil {
		return err
	}
	...
	r.API.Telemetry.TrackOAuthComplete(string(app.AppID), r.ActingUserID())
