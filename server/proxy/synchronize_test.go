...
func TestSynchronize(t *testing.T) {
	...
	for _, tc := range []TC{
		...
		t.Run(tc.name, func(t *testing.T) {
			p := createTestProxy(tc.inputApps, tc.inputManifests)
...
func createTestProxy(a map[apps.AppID]*apps.App, m map[apps.AppID]*apps.Manifest) *Proxy {
	...
	mutex, _ := cluster.NewMutex(testAPI, config.KVClusterMutexKey)
