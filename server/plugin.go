...
func (p *Plugin) OnActivate() error {
	...
	p.proxy = proxy.NewService(p.mm, p.log, p.conf, p.aws, conf.AWSS3Bucket, p.store, mutex, p.httpOut, p.i18n)
	...
	p.httpIn = httpin.NewService(mux.NewRouter(), p.mm, p.log, p.conf, p.proxy, p.appservices, p.i18n,
		dialog.Init,
		restapi.Init,
		gateway.Init,
		http_hello.Init,
	...
	p.command, err = command.MakeService(p.mm, p.log, p.conf, p.proxy, p.httpOut, p.i18n)
	if err != nil {
