...
func (a *builtinApp) settingsSave(r *incoming.Request, creq apps.CallRequest) apps.CallResponse {
	...
	sc.DeveloperMode = apps.DeployType(creq.GetValue(fDeveloperMode, "")) == "true"
	sc.AllowHTTPApps = apps.DeployType(creq.GetValue(fAllowHTTPApps, "")) == "true"
	...
	return apps.NewTextResponse("Saved %v", creq.Values)
}
