...
func TestCreateOAuth2State(t *testing.T) {
	...
	api.On("KVSetWithOptions", mock.Anything, mock.Anything, mock.Anything).Once().Run(func(args mock.Arguments) {
		require.Regexp(t, OAuth2StatePrefix+stateRE, args.String(0))
		data, _ := args.Get(1).([]byte)
		require.Regexp(t, stateRE, string(data))
	}).Return(true, nil)

	r := incoming.NewRequest(conf, nil, "test", "reqid").WithActingUserID(userID).WithSourceAppID("some_app_id")
	...
	mismatchedKey := OAuth2StatePrefix + mismatchedState
...
func TestOAuth2User(t *testing.T) {
	...
	conf, api := config.NewTestService(nil)
	s := OAuth2Store{}
	r := incoming.NewRequest(conf, nil, "test", "reqid").WithActingUserID(userID).WithSourceAppID(appID)
