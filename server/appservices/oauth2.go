...
func (a *AppServices) StoreOAuth2App(r *incoming.Request, data []byte) error {
	...
	r.Config().MattermostAPI().Frontend.PublishWebSocketEvent(config.WebSocketEventRefreshBindings, map[string]any{}, &model.WebsocketBroadcast{})
...
func (a *AppServices) StoreOAuth2User(r *incoming.Request, data []byte) error {
	...
	r.Config().MattermostAPI().Frontend.PublishWebSocketEvent(config.WebSocketEventRefreshBindings, map[string]any{}, &model.WebsocketBroadcast{UserId: actingUserID})
	return nil
}
