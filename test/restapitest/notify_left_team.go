...
func notifyUserLeftTeam(_ *Helper) *notifyTestCase {
	return &notifyTestCase{
		...
		expected: func(th *Helper, level apps.ExpandLevel, appclient appClient, data apps.ExpandedContext) (apps.Subject, apps.ExpandedContext) {
			...
			return apps.SubjectUserLeftTeam, ec
		},
	}
}

func notifyBotLeftTeamLegacy(th *Helper) *notifyTestCase {
	return notifyLeftTeamImpl(th, apps.SubjectBotLeftTeam, apps.SubjectBotLeftTeam, true, []appClient{th.asAdmin, th.asUser, th.asUser2})
}

func notifyBotLeftTeamRemapped(th *Helper) *notifyTestCase {
	return notifyLeftTeamImpl(th, apps.SubjectBotLeftTeam, apps.SubjectSelfLeftTeam, false, []appClient{th.asAdmin, th.asUser, th.asUser2})
}

func notifySelfLeftTeam(th *Helper) *notifyTestCase {
	return notifyLeftTeamImpl(th, apps.SubjectSelfLeftTeam, apps.SubjectSelfLeftTeam, false, []appClient{th.asAdmin})
}

func notifyLeftTeamImpl(th *Helper, subjectIn, subjectOut apps.Subject, testFlag bool, except []appClient) *notifyTestCase {
	return &notifyTestCase{
		except:           except,
		...
		trigger: func(th *Helper, data apps.ExpandedContext) apps.ExpandedContext {
			th.removeTeamMember(data.Team, data.ActingUser)
			data.TeamMember = th.getTeamMember(data.Team.Id, data.ActingUser.Id)
			data.TeamMember.Roles = ""
			return data
		},
		expected: func(th *Helper, level apps.ExpandLevel, appclient appClient, data apps.ExpandedContext) (apps.Subject, apps.ExpandedContext) {
			ec := apps.ExpandedContext{
